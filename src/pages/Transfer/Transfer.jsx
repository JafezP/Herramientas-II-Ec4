...
const Transfer = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      Swal.fire("Error", "Error al realizar la transferencia: " + error.response.data, "error");
    }
  };
  ...
  useEffect(() => {
    if (originAccount) {
      const fetchDestinationAccounts = async () => {
        try {
          const response = await axios.get(`api/cuentas/destino/${originAccount}`);
...
>
  <div className="border border-gray-200 rounded-md shadow w-full max-w-md lg:max-w-lg p-6 lg:p-10 bg-white">
    ...
    <div className="grid grid-cols-1 gap-2">
      ...
      <select
        {...register("cuentaOrigen", { required: "Selecciona una cuenta" })}
        value={originAccount}
      ...
      {errors.cuentaOrigen && <span className="text-red-500 text-xs">{errors.cuentaOrigen.message}</span>}
    </div>
    ...
    <div className="grid grid-cols-1 gap-2 mt-5">
      ...
      >
        ...
        {destinationAccounts.map((acc) => (
          <option key={acc.id} value={acc.id}>{acc.tipoCuenta}</option>
        ))}
      </select>
      {errors.cuentaDestino && <span className="text-red-500 text-xs">{errors.cuentaDestino.message}</span>}
    </div>
    ...
    <div className="flex gap-4 mt-10 border-dashed border border-gray-200 rounded-md p-3">
      ...
      <p className="text-sm">
        ...
        <span className="block font-black text-blue-500 hover:underline cursor-pointer">Conoce más</span>
      </p>
    </div>
    ...
    <div className="flex justify-between mt-10">
      ...
      <button type="submit" className="flex items-center gap-2 text-blue-500 font-semibold cursor-pointer transform hover:scale-105 transition-transform">
        Transferir
